...
numrobots = int(os.getenv('NUM_PLANTS'))
robothost = os.getenv('PLANT_HOSTNAME', 'http://127.0.0.1:8000/')
etcdhost = os.getenv('ETCD_HOSTNAME', '127.0.0.1')

f = Fernet(mykey)
# mytoken = f.encrypt(bytes("0", encoding="utf8"))
allrobots = set([str(i) for i in range(numrobots)])
...
@cross_origin()
def login():
    requested_robot = (allrobots-set([i['value'] for i in client.get(etcdnode)._children])).pop()
    response = client.write(etcdnode,
                            requested_robot,
                            append=True,
                            ttl=ttl)
    assert response.value == requested_robot, "The requested robot wasn't assigned"
...
def _proxy(token="", req=""):
    robot = f.decrypt(bytes(token, encoding="utf8"), ttl=ttl).decode("utf-8")
    resp = requests.request(
        method=request.method,
        url="{}{}/{}?{}".format(robothost, robot, req, request.query_string.decode("utf-8")),
